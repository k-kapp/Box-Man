...
namespace BoxMan
{
    public class LevelDesigner : StateBase
    {
        ...
        public LevelDesigner(int rows, int cols, GameMgr gameMgr) : base(gameMgr)
        {
            ...
            _grid = new BoxMan.PuzzleGrid(rows, cols, _gameMgr);
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                {
                    _grid[row, col] = new BoxMan.Tile(false, Occpr.VOID);
                }
            }
        ...
        private void _makeGridResizePopup(object sender, ButtonEventArgs args)
        {
            ...
            popupInput.AddLabel(new BoxMan.XNALabel("Number of columns (maximum " + _maxCols + "): ", _gameMgr.Content.Load<SpriteFont>("Courier New"), new Vector2(50, 15), 1.5f, popupInput));
            var columnsTextField = new BoxMan.TextField(50, 50, 200, 30, popupInput);
            columnsTextField.Text = _cols.ToString();
            popupInput.AddLabel(new BoxMan.XNALabel("Number of rows (maximum " + _maxRows + "): ", _gameMgr.Content.Load<SpriteFont>("Courier New"), new Vector2(50, 95), 1.5f, popupInput));
            var rowsTextField = new BoxMan.TextField(50, 150, 200, 30, popupInput);
            rowsTextField.Text = _rows.ToString();

            ButtonEventArgs eventArgs = new BoxMan.ButtonEventArgs();
        ...
        {
            _designForm = new BoxMan.XNAForm(0, 0, _gameMgr.ScreenWidth, _gameMgr.ScreenHeight, this, "", false);
        ...
        private void _makeDesignGrid()
        {
            ...
            _designGridForm = new BoxMan.XNAForm(100, 100, _cols * _gridTileSize + 2*borderWidth, _rows * _gridTileSize + 2*borderWidth, _designForm, "", false);
            ...
            for (int row = 0; row < _rows; row++)
            {
                for (int col = 0; col < _cols; col++)
                {
                        ...
                        && (row < tempPaintButtonGrid.GetLength(0) && col < tempPaintButtonGrid.GetLength(1)))
                    {
                        ...
                        button = new BoxMan.PaintButton(col * _gridTileSize, row * _gridTileSize, _gridTileSize, _gridTileSize, _designGridForm);
